...
import me.nereo.multi_image_selector.MultiImageView;
...
public class FolderAdapter extends BaseAdapter {
        ...
        if (holder != null) {
            if(i == 0){
                ...
                if(mFolders.size()>0){
                    ...
                    if (f != null) {
                        int size = (int)(72*mContext.getResources().getDisplayMetrics().density);
                        LoadImageParams.Builder builder = LoadImageParams.newBuilder();
                        builder.uri("file://" + f.cover.path)
                                .width(size)
                                .height(size)
                                .baseImageView(holder.cover)
                                .defaultholder(R.drawable.mis_default_error)
    ...
    class ViewHolder{
        MultiImageView cover;
        TextView name;
        TextView path;
        TextView size;
        ImageView indicator;
        ViewHolder(View view){
            cover = (MultiImageView)view.findViewById(R.id.cover);
        ...
        void bindData(Folder data) {
            ...
            if (data.cover != null) {
                // 显示图片
                int size = (int)(mContext.getResources().getDisplayMetrics().density*72);
                LoadImageParams.Builder builder = LoadImageParams.newBuilder();
                builder.uri("file://" + data.cover.path)
                        .baseImageView(cover)
                        .width(size)
                        .height(size)
                        .scaleType(ImageView.ScaleType.CENTER_CROP)
